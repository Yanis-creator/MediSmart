from util import base64_to_pil , malign, benin, patche_map 
...
EXAM_LIST_PATH='./src/utilities/data_new'
...
def visualize_example(input_img, saliency_maps, true_segs,
    ...
    subfigure.set_title("image d'entrée")
    subfigure.axis('off')
...
def run_model(model, exam_list, parameters, turn_on_visualization):
    ...
    with torch.no_grad():
        ...
        for datum in tqdm.tqdm(exam_list):
            for view in ['L-CC']:# VIEWS.LIST:
        
                short_file_path = datum[view][0]
                loaded_image = parameters["image_path"]
...
def crop_ouput(img_path,save_path):
...
def predict3():
    if request.method == 'POST':
        ...
        cover = resizeimage.resize_cover(img, [1920, 2944])
        cover = np.array(cover, dtype=np.float64)
        parameters = {
            "device_type": DEVICE_TYPE,
            "gpu_number":GPU_NUMBER,
            "max_crop_noise": (100, 100),
            "max_crop_size_noise": 100,
            "image_path":cover,#CROPPED_IMAGE_PATH,
        ...
        file_name = './sample_output/predictions.csv'
        df = pd.read_csv(file_name, nrows=1)
        x = int(10000*(df.at[0, 'benign_pred']))
        y = int(10000*(df.at[0, 'malignant_pred']))
        x = str(x/100)
        y = str(y/100)
        result = 'Begnin : '+x+'%    Malign : ' + y + '%'
        #result = 'you can see your results'
        result = result.replace('_', ' ').capitalize()
        return jsonify(result=result)
    img_path = './sample_output/visualization/0_L-CC.png'
    save_path1= './static/images/malin.png'
    save_path2= './static/images/benin.png'
    save_path3= './static/images/patche_map.png'
    malign(img_path,save_path1)
    benin(img_path,save_path2)
    patche_map(img_path,save_path3)
    return None
...
def see3():
    img_path = './sample_output/visualization/0_L-CC.png'
    save_path= 'L-CC-cropped.png'
    crop_ouput(img_path,save_path)
...
def patches2():
    img_path = './sample_output/visualization/0_L-CC.png'
    output_patches(img_path)
    save_path = './outputs/patches.png'
    return send_file(save_path, mimetype='image/png')
